...
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Back Button -->
    <a href="{{ route('anggota.laporan') }}" class="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
    ...
    <div class="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
        @if($laporan->bukti_foto)
            <div class="aspect-video bg-muted">
                <img src="{{ asset('storage/' . $laporan->bukti_foto) }}" alt="Foto Kerusakan" class="w-full h-full object-cover">
            </div>
        @endif
        ...
        <div class="p-6 space-y-6">
            <div class="flex items-start justify-between">
                <div>
                    <h2 class="font-display font-bold text-xl text-foreground">{{ $laporan->fasilitas->nama_fasilitas }}</h2>
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <p class="font-medium text-foreground">{{ $laporan->created_at->format('d F Y, H:i') }}</p>
                </div>
                <div>
                    ...
                    @foreach($laporan->penanganan as $penanganan)
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1 pb-4 {{ !$loop->last ? 'border-b border-border' : '' }}">
                                ...
                                <p class="text-sm text-muted-foreground">{{ $penanganan->catatan_penanganan }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
